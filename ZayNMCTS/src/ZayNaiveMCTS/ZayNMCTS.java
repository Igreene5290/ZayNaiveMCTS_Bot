...
public class ZayNMCTS extends AIWithComputationBudget implements InterruptibleAI {
    ...
    // Reward multiplier for faster strategies
    public float reward_mult = 0.6f;
    
    public int global_strategy = ZayNMCTSNode.E_GREEDY;
    ...
    public ZayNMCTS(UnitTypeTable utt) {
        this(100,-1,100,10,
             0.3f, 0.0f, 0.4f, 0.8f,
             new RandomBiasedAI(),
    ...
    public ZayNMCTS(int available_time, int max_playouts, int lookahead, int max_depth, 
    ...
    public ZayNMCTS(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, float rewardmult, AI policy, EvaluationFunction a_ef, boolean fensa) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public ZayNMCTS(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, int a_global_strategy, float rewardmult, AI policy, EvaluationFunction a_ef, boolean fensa) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    @Override
    public AI clone() {
        return new ZayNMCTS(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME, MAX_TREE_DEPTH, epsilon_l, discount_l, epsilon_g, discount_g, epsilon_0, discount_0, reward_mult, playoutPolicy, ef, forceExplorationOfNonSampledActions);
    }    
    ...
    public void startNewComputation(int a_player, GameState gs) throws Exception {
        player = a_player;
        current_iteration = 0;
        tree = new ZayNMCTSNode(player, 1-player, gs, null, ef.upperBound(gs), current_iteration++, forceExplorationOfNonSampledActions, reward_mult);
...
//        System.out.println("HL: " + count + " time: " + (System.currentTimeMillis() - start) + " (" + available_time + "," + max_playouts + ")");
    ...
    public boolean iteration(int player) throws Exception {
        ...
        ZayNMCTSNode leaf = tree.selectLeaf(player, 1-player, epsilon_l, epsilon_g, epsilon_0, global_strategy, MAX_TREE_DEPTH, current_iteration++);
...
//            System.out.println(total_runs + " - " + epsilon_0 + ", " + epsilon_l + ", " + epsilon_g);
    ...
    public PlayerAction getBestActionSoFar() {
        ...
        if (idx==-1) {
            if (DEBUG>=1) System.out.println("ZayNMCTS no children selected. Returning an empty asction");
            return new PlayerAction();
        }
        if (DEBUG>=2) tree.showNode(0,1,ef);
        if (DEBUG>=1) {
            ZayNMCTSNode best = (ZayNMCTSNode) tree.children.get(idx);
            System.out.println("ZayNMCTS selected children " + tree.actions.get(idx) + " explored " + best.visit_count + " Avg evaluation: " + (best.accum_evaluation/((double)best.visit_count)));
        }
...
//            }
        ...
        for(int i = 0;i<tree.children.size();i++) {
            ZayNMCTSNode child = (ZayNMCTSNode)tree.children.get(i);
            if (DEBUG>=2) {
